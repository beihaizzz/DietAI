...
class PersonalizedHomePage extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<PersonalizedHomePage> createState() => _PersonalizedHomePageState();
}
...
class _PersonalizedHomePageState extends ConsumerState<PersonalizedHomePage> {
    ...
    try {
      ...
      setState(() {
        ...
        if (results[1].success && results[1].data != null) {
          _diseases = results[1].data!;
        }
        if (results[2].success && results[2].data != null) {
          _allergies = results[2].data!;
        }
  ...
  UserType _determineUserType() {
    ...
    if (_userProfile?.healthGoals?.any((goal) => 
        goal.goalType == 'weight_loss' || goal.goalType == 'muscle_gain') == true) {
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            Row(
              children: [
                const Icon(Icons.medical_information, color: Colors.red, size: 24),
  ...
  Widget _buildMedicalAlertsCard() {
    final severeDiseases = _diseases.where((d) => d.isCurrent && d.severityLevel >= 3).toList();
    final severeAllergies = _allergies.where((a) => a.severityLevel >= 3).toList();
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 8),
            ...severeDiseases.map((disease) => Text(
              '• ${disease.diseaseName}：请严格遵循饮食限制',
              style: const TextStyle(color: Colors.orange),
            )).toList(),
            ...severeAllergies.map((allergy) => Text(
  ...
  Widget _buildMealItem(String mealType, String food, String time, int calories) {
    return Padding(
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 16),
            ...['严格控制钠盐摄入，每日不超过6克', '避免高糖食物，选择低升糖指数食材', '定时服药，监测血压变化', '如有不适请及时就医'].map((tip) {
              return Padding(
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 16),
            ...['运动前后记得补充水分', '增加蛋白质摄入，促进肌肉恢复', '控制总热量，创造合理的热量缺口', '保证充足睡眠，有助于身体恢复'].map((tip) {
              return Padding(
                padding: const EdgeInsets.only(bottom: 8),
                child: Row(
                  crossAxisAlignment: CrossAxisAlignment.start,
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 16),
            ...['多吃新鲜蔬果，补充维生素和纤维', '保持饮水量，每日至少8杯水', '控制油盐糖，养成清淡饮食习惯', '规律作息，有助于新陈代谢'].map((tip) {
              return Padding(
                padding: const EdgeInsets.only(bottom: 8),
                child: Row(
                  crossAxisAlignment: CrossAxisAlignment.start,
