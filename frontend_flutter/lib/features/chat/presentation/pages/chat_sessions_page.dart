...
class _ChatSessionsPageState extends ConsumerState<ChatSessionsPage> {
  ...
  Future<void> _deleteSession(ChatSessionSummary session) async {
    ...
    if (confirmed == true) {
      try {
        final response = await _chatService.deleteSession(sessionId: session.id);
  ...
  Widget _buildSessionTypeSelector() {
    final sessionTypes = [
      {'type': 1, 'name': '营养咨询', 'icon': Icons.restaurant, 'color': Colors.green},
      {'type': 2, 'name': '健康评估', 'icon': Icons.health_and_safety, 'color': Colors.blue},
      {'type': 3, 'name': '食物识别', 'icon': Icons.camera_alt, 'color': Colors.orange},
      {'type': 4, 'name': '运动建议', 'icon': Icons.fitness_center, 'color': Colors.purple},
    ];
