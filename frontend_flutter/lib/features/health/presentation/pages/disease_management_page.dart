...
class DiseaseManagemenetPage extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<DiseaseManagemenetPage> createState() => _DiseaseManagemenetPageState();
}
...
class _DiseaseManagemenetPageState extends ConsumerState<DiseaseManagemenetPage>
    ...
    try {
      ...
      setState(() {
        if (results[0].success && results[0].data != null) {
          _diseases = results[0].data!;
        }
        if (results[1].success && results[1].data != null) {
          _allergies = results[1].data!;
        }
        _isLoading = false;
      });
    } catch (e) {
            ...
            Row(
              children: [
                Container(
                  ...
                  decoration: BoxDecoration(
                    color: disease.isCurrent ? Colors.red.withOpacity(0.1) : Colors.grey.withOpacity(0.1),
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 16),
            ...['定期监测健康指标，及时调整饮食方案', '保持均衡营养，不要过度限制', '如有不适，请及时咨询医生', '记录饮食反应，帮助优化管理方案', '保持规律作息，适量运动'].map((tip) {
              return Padding(
  ...
  IconData _getAllergenTypeIcon(int type) {
    switch (type) {
      case 1: return Icons.grass; // 食物
      case 2: return Icons.eco; // 环境
      case 3: return Icons.medical_services; // 药物
